		...
		<ul class="py-1">
			...
			<li v-if="items.length !== 0" v-for="(item, index) in items" :key="item.id" class="py-3 my-3 bg-soft-black-2 min-h-32 flex justify-between h-full px-6 sm:px-16 md:px-32 lg:px-56 xl:px-8 2xl:px-36 border-b-4 border-soft-black-1/50">
				...
				<div class="flex flex-col justify-between py-1">
					<button type="button" @click="confirm(item.id)" class="text-white bg-red-600 border-b-4 border-red-800 font-medium rounded-lg text-sm w-8 h-8 justify-center px-1 text-center inline-flex relative bottom-1 items-center me-2">
					...
					<button type="button" @click="openItemEdit(item)" class="text-white bg-gray-500 border-b-4 border-gray-700 w-8 h-8 font-medium rounded-lg text-sm py-1 text-center inline-flex items-center me-2">
					...
					<delete-confirmation :show="showConfirmation" @confirmed="deleteItem(itemToDelete)" @cancelled="showConfirmation = false"/>
				</div>
			</li>
		</ul>
...
export default {
	setup() {
		...
		const confirm = (itemId) => {
			itemToDelete.value = itemId; // Set the spell to delete
