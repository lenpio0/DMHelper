        ...
        <ul class="py-1">
            ...
            <li v-if="char_infos.length !== 0" v-for="(char_info, index) in char_infos" :key="char_info.id" class="p-3 my-3 bg-soft-black-2 min-h-32 flex justify-between h-full sm:px-16 md:px-32 lg:px-56 xl:px-[360px] 2xl:px-[480px] border-b-4 border-soft-black-1/50">
				...
				<div class="flex flex-col justify-between py-1">
                    <button v-if="authRole !== 'player'" @click="confirm(char_info.id)" class="text-white bg-red-600 border-b-4 border-red-800 font-medium rounded-lg text-sm w-8 h-8 justify-center px-1 text-center inline-flex relative bottom-1 items-center me-2">
                ...
                <button v-if="authRole !== 'player'" @click="openCharInfoEdit(char_info)" class="text-white bg-gray-500 border-b-4 border-gray-700 w-8 h-8 font-medium rounded-lg text-sm py-1 text-center inline-flex items-center me-2">
                ...
                <delete-confirmation :show="showConfirmation" @confirmed="deleteCharInfo(infoToDelete)" @cancelled="showConfirmation = false"/>
                </div>
            </li>
        </ul>
...
export default {
    setup() {
        ...
        const confirm = (infoId) => {
			infoToDelete.value = infoId; // Set the spell to delete
