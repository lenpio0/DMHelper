...
<template>
    <div class="z-50 fixed backdrop-blur-sm top-0 bottom-0 left-0 right-0 m-auto flex items-center justify-center bg-black/50">
        <div class="w-80 sm:w-[24rem]">
            <h2 class='font-albert text-white bg-soft-black-3 w-56 pt-2 pb-1.5 m-auto rounded-t-lg text-center text-xl font-bold'>Edit Character Info</h2>
            <form @submit.prevent="editInfo" class="bg-soft-black-2 flex flex-col px-2 py-4 rounded-3xl">
                <textarea v-model="info" required rows="3" class="m-3 rounded-2xl text-black"></textarea>
                <div class="my-2 mx-auto">
                    <label class="flex items-center cursor-pointer rounded pb-2 border-b-4 border-soft-black-2 mx-auto">
                    <input type="checkbox" v-model="is_secret" :checked="is_secret" class="sr-only peer">
                ...
                <div class="flex justify-around py-2">
                    <button @click="closeInfoEdit" class="bg-red-600 box-content border-b-4 border-red-800 font-medium rounded-lg text-sm w-14 h-9 justify-center px-1 text-center inline-flex left-[2.1rem] items-center">
